...
const displayNextWords = () => {
    const nextWords = words.slice(currentIndex, currentIndex + 40).join(" "); // Change to 40 words
...
const handleTestCompletion = () => {
    ...
    const currentWords = words.slice(currentIndex, currentIndex + 40); // Get the current 40 words
...
const highlightEnabled = document.getElementById("highlight_toggle").checked;
 ...
 const typedWords = typedText.split(" ");
    ...
    if (typedText) {
        const chunkIndex = Math.floor(currentIndex / 40); // Calculate the chunk index based on the current index
...
document.addEventListener('keydown', function(event) {
    if (event.key === 'ArrowUp') {
        if (currentChunkIndex > 0) {
            ...
            currentIndex = currentChunkIndex * 40; // Update currentIndex based on chunk index
...
function updateCurrentChunkDisplay() {
    const totalChunks = Math.ceil(words.length / 40); // Assuming 40 words per chunk
...
document.getElementById("typed_text").addEventListener("keydown", (e) => {
    ...
    // Handle Enter key functionality
    if (e.key === "Enter") {
        const typedWords = typedText.trim().split(" ").filter(word => word.length > 0); // Split and filter out empty strings
        const currentWords = words.slice(currentIndex, currentIndex + 40); // Get the current 40 words

        // Check if more than 35 words are typed
        if (typedWords.length > 35) {
            // Compare typed words with current words
            let correctCount = 0;
            let wrongCount = 0;
            ...
            currentIndex += 40; // Increment by 40 to move to the next set of words
            ...
            alert("Please type more than 35 words before pressing Enter.");
        }
    }
});
...
const highlightText = (typedText, currentWords) => {
    const typedWords = typedText.split(" ");
    const highlightedText = currentWords.map((word, index) => {
        return typedWords[index] === word ? `<span class="correct">${word}</span>` : `<span class="wrong">${word}</span>`;
    }).join(" ");
    document.getElementById("test_text").innerHTML = highlightedText;
};
